...
const forecast = (address, callback) => {
    ...
    request({ url: url, json: true }, (error, response) => {
        if (error) {
            ...
            callback(undefined, 'It is ' + response.body.current.weather_descriptions + ' in ' + response.body.location.name + ', ' + response.body.location.region + '. It is currently ' + response.body.current.temperature + ' degrees out. And It feelslike ' + response.body.current.feelslike + ' And Humidity is ' + response.body.current.humidity)
        }
    })
}
